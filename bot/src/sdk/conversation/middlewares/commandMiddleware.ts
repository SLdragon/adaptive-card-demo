...
import { Activity, ActivityTypes, CardFactory, Middleware, TurnContext } from "botbuilder";
import { PetLinkUnfurlingHandler } from "../../../petLinkUnfurlingHandler";
...
export class CommandResponseMiddleware implements Middleware {
  ...
  public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
    if (context.activity.name === "composeExtension/queryLink") {
      ...
      const hanlder = new PetLinkUnfurlingHandler();
      const result = await hanlder.handleLinkReceived(context, context.activity.value.url)
